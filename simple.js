...
exports.makeWASocket = (connectionOptions, options = {}) => {
    let XeonBotInc = (global.opts['legacy'] ? makeWALegacySocket : makeWASocket)(connectionOptions)
    // XeonBotInc.ws.on('CB:stream:error', (stream) => {
    //     const { code } = stream || {}
    //     if (code == '401') XeonBotInc.ev.emit('connection.update', {
    ...
    // Load Group Message
    XeonBotInc.loadAllMessages = (messageID) => {
    ...
    if (!XeonBotInc.chats) XeonBotInc.chats = {}
    ...
    function updateNameToDb(contacts) {
        ...
        for (const contact of contacts) {
            ...
            if (!id) continue
            let chats = XeonBotInc.chats[id]
            if (!chats) chats = XeonBotInc.chats[id] = { id }
            XeonBotInc.chats[id] = {
                ...chats,
                ...({
    ...
    XeonBotInc.ev.on('chats.set', async ({ chats }) => {
        for (const { id, name, readOnly } of chats) {
            ...
            if (isGroup) {
                const metadata = await XeonBotInc.groupMetadata(id).catch(_ => null)
    ...
    XeonBotInc.ev.on('group-participants.update', async function updateParticipantsToDb({ id, participants, action }) {
        id = XeonBotInc.decodeJid(id)
        if (!(id in XeonBotInc.chats)) XeonBotInc.chats[id] = { id }
        XeonBotInc.chats[id].isChats = true
        const groupMetadata = await XeonBotInc.groupMetadata(id).catch(_ => null)
    ...
    XeonBotInc.ev.on('groups.update', async function groupUpdatePushToDb(groupsUpdates) {
        for (const update of groupsUpdates) {
            const id = XeonBotInc.decodeJid(update.id)
            if (!id) continue
            const isGroup = id.endsWith('@g.us')
            if (!isGroup) continue
            let chats = XeonBotInc.chats[id]
    ...
    XeonBotInc.ev.on('chats.upsert', async function chatsUpsertPushToDb(chatsUpsert) {
        console.log({ chatsUpsert })
        const { id, name } = chatsUpsert
        if (!id) return
        let chats = XeonBotInc.chats[id] = { ...XeonBotInc.chats[id], ...chatsUpsert, isChats: true }
        const isGroup = id.endsWith('@g.us')
        if (isGroup) {
            const metadata = await XeonBotInc.groupMetadata(id).catch(_ => null)
            if (metadata) {
            ...
            const groups = await XeonBotInc.groupFetchAllParticipating().catch(_ => ({})) || {}
            for (const group in groups) XeonBotInc.chats[group] = { id: group, subject: groups[group].subject, isChats: true, metadata: groups[group] }
        }
    })
    XeonBotInc.ev.on('presence.update', async function presenceUpdatePushToDb({ id, presences }) {
        const sender = Object.keys(presences)[0] || id
        const _sender = XeonBotInc.decodeJid(sender)
        const presence = presences[sender]['lastKnownPresence'] || 'composing'
        let chats = XeonBotInc.chats[_sender]
        if (!chats) chats = XeonBotInc.chats[_sender] = { id: sender }
        chats.presences = presence
        if (id.endsWith('@g.us')) {
            let chats = XeonBotInc.chats[id]
            if (!chats) {
                const metadata = await XeonBotInc.groupMetadata(id).catch(_ => null)
                if (metadata) chats = XeonBotInc.chats[id] = { id, subject: metadata.subject, metadata }
            }
            chats.isChats = true
        }
    })
  ...
  XeonBotInc.delay = (ms) => new Promise((resolve) => setTimeout(resolve, ms))
  ...
  /**
    ...
    */
    ...
    XeonBotInc.adReply = (jid, text, title = '', body = '', buffer, source = '', quoted, options) => {
                let { data } = XeonBotInc.getFile(buffer, true)
                return XeonBotInc.sendMessage(jid, { text: text, 
                    contextInfo: {
                        mentionedJid: XeonBotInc.parseMention(text),
                        externalAdReply: {
    ...
    */
     ...
     * @param {Object} options 
     */
     XeonBotInc.sendContact = async (jid, data, quoted, options) => {
                if (!Array.isArray(data[0]) && typeof data[0] === 'string') data = [data]
                let contacts = []
                for (let [number, name] of data) {
                    number = number.replace(/[^0-9]/g, '')
                    let njid = number + '@s.whatsapp.net'
                    let biz = await XeonBotInc.getBusinessProfile(njid).catch(_ => null) || {}
                    let vcard = `
BEGIN:VCARD
VERSION:3.0
FN:${name.replace(/\n/g, '\\n')}
...
PHOTO;BASE64:${(await XeonBotInc.getFile(await XeonBotInc.profilePictureUrl(njid)).catch(_ => ({})) || {}).number?.toString('base64')}
...
`.trim()
      ...
      XeonBotInc.sendList = async (jid, header, footer, separate, buttons, rows, quoted, options) => {
    ...
    XeonBotInc.reply = (jid, text = '', quoted, options) => {
        return Buffer.isBuffer(text) ? XeonBotInc.sendFile(jid, text, 'file', '', quoted, false, options) : XeonBotInc.sendMessage(jid, { ...options, text, mentions: XeonBotInc.parseMention(text) }, { quoted, ...options, mentions: XeonBotInc.parseMention(text) })
    }
    
    XeonBotInc.resize = async (image, width, height) => {
                let oyy = await Jimp.read(image)
                let kiyomasa = await oyy.resize(width, height).getBufferAsync(Jimp.MIME_JPEG)
                return kiyomasa
            }
    
    XeonBotInc.fakeReply = (jid, text = '', fakeJid = XeonBotInc.user.jid, fakeText = '', fakeGroupJid, options) => {
        return XeonBotInc.sendMessage(jid, { text: text }, { ephemeralExpiration: 86400, quoted: { key: { fromMe: fakeJid == XeonBotInc.user.jid, participant: fakeJid, ...(fakeGroupJid ? { remoteJid: fakeGroupJid } : {}) }, message: { conversation: fakeText }, ...options } })
    }
    XeonBotInc.reply1 = async (jid, text, quoted, men) => {
        return XeonBotInc.sendMessage(jid, {
            text: text, jpegThumbnail: await (await fetch(thumbr1)).buffer(), mentions: men
        }, { quoted: quoted, ephemeralExpiration: 86400 })
    }
    XeonBotInc.reply2 = async (jid, text, media, quoted, men) => {
        return XeonBotInc.sendMessage(jid, {
    ...
    * @returns 
    */
    XeonBotInc.cMod = (jid, message, text = '', sender = XeonBotInc.user.jid, options = {}) => {
    ...
    XeonBotInc.loadMessage = XeonBotInc.loadMessage || (async (messageID) => {
        return Object.entries(XeonBotInc.chats)
            .filter(([_, { messages }]) => typeof messages === 'object')
            .find(([_, { messages }]) => Object.entries(messages)
                .find(([k, v]) => (k === messageID || v.key?.id === messageID)))
     ...
     */
    XeonBotInc.downloadM = async (m, type, saveToFile) => {
        ...
        if (saveToFile) var { filename } = await XeonBotInc.getFile(buffer, true)
     ...
     */
    XeonBotInc.chatRead = async (jid, participant = XeonBotInc.user.jid, messageID) => {
    ...
    if (!chat || chat === 'status@broadcast') return
        const emitGroupUpdate = (update) => {
            XeonBotInc.ev.emit('groups.update', [{ id: chat, ...update }])
        }
    ...
    /*XeonBotInc.processMessageStubType = async (m) => {
        ...
        const chat = XeonBotInc.decodeJid(m.key.remoteJid || m.message[mtype] && m.message[mtype].groupId || '')
        const isGroup = chat.endsWith('@g.us')
        if (!isGroup) return
        let chats = XeonBotInc.chats[chat]
        if (!chats) chats = XeonBotInc.chats[chat] = { id: chat }
...
XeonBotInc.setBio = async (status) => {
    ...
    /*XeonBotInc.pushMessage = async (m) => {
        ...
        for (const message of m) {
            try {
                // if (!(message instanceof proto.WebMessageInfo)) continue // https://github.com/adiwajshing/Baileys/pull/696/commits/6a2cb5a4139d8eb0a75c4c4ea7ed52adc0aec20f
                if (!message) continue
                if (message.messageStubType) XeonBotInc.processMessageStubType(message).catch(console.error)
                let mtype = Object.keys(message.message || {})
                mtype = mtype[mtype[0] === 'messageContextInfo' && mtype.length == 2 ? 1 : 0]
                const chat = XeonBotInc.decodeJid(message.key.remoteJid || message.message[mtype] && message.message[mtype].groupId || '')
                const isGroup = chat.endsWith('@g.us')
                let chats = XeonBotInc.chats[chat]
                if (!chats) {
                    if (isGroup) {
                    ...
                    chats = XeonBotInc.chats[chat] = { id: chat, ...(XeonBotInc.chats[chat] || {}) }
                }
                let metadata, sender
                if (isGroup) {
                    if (!chats.subject || !chats.metadata) {
                    ...
                    sender = XeonBotInc.decodeJid(message.fromMe && XeonBotInc.user.id || message.participant || message.key.participant || chat || '')
                    if (sender !== chat) {
                        let chats = XeonBotInc.chats[sender]
                        if (!chats) chats = XeonBotInc.chats[sender] = { id: sender }
                        if (!chats.name) chats.name = message.pushName || chats.name || ''
                ...
                chats.isChats = true
                const fromMe = message.key.fromMe || areJidsSameUser(chat, XeonBotInc.user.id)
                if (!chats.messages) chats.messages = {}
     ...
     * @returns 
     */
    XeonBotInc.getBuffer = async (url, options) => {
        try {
            options ? options : {}
 ...
 */
 exports.smsg = (XeonBotInc, m, hasParent) => {
    if (!m) return m
    ...
    if (m.key) {
        ...
        m.chat = XeonBotInc.decodeJid(m.key.remoteJid || message.message?.senderKeyDistributionMessage?.groupId || '')
        m.isGroup = m.chat.endsWith('@g.us')
        m.sender = XeonBotInc.decodeJid(m.key.fromMe && XeonBotInc.user.id || m.participant || m.key.participant || m.chat || '')
        m.fromMe = m.key.fromMe || areJidsSameUser(m.sender, XeonBotInc.user.id)
    }
    if (m.message) {
        ...
        if (m.mtype == 'protocolMessage' && m.msg.key) {
            ...
            m.msg.key.fromMe = XeonBotInc.decodeJid(m.msg.key.participant) === XeonBotInc.decodeJid(XeonBotInc.user.id)
            if (!m.msg.key.fromMe && m.msg.key.remoteJid === XeonBotInc.decodeJid(XeonBotInc.user.id)) m.msg.key.remoteJid = m.sender
        }
        ...
        if (m.quoted) {
            ...
            m.quoted.chat = XeonBotInc.decodeJid(m.msg.contextInfo.remoteJid || m.chat || m.sender)
            m.quoted.isBaileys = m.quoted.id && m.quoted.id.length === 16 || false
            m.quoted.sender = XeonBotInc.decodeJid(m.msg.contextInfo.participant)
            m.quoted.fromMe = m.quoted.sender === XeonBotInc.user.jid
            m.quoted.text = m.quoted.text || m.quoted.caption || m.quoted.contentText || ''
            m.quoted.name = XeonBotInc.getName(m.quoted.sender)
            ...
            m.getQuotedObj = m.getQuotedMessage = async () => {
                ...
                let q = M.fromObject(await XeonBotInc.loadMessage(m.quoted.id) || vM)
                return exports.smsg(XeonBotInc, q)
            }
            if (m.quoted.url || m.quoted.directPath) m.quoted.download = (saveToFile = false) => XeonBotInc.downloadM(m.quoted, m.quoted.mtype.replace(/message/i, ''), saveToFile)
...
/*exports.smsg = (conn, m, hasParent) => {
    ...
    if (m.key) {
        ...
        m.chat = XeonBotInc.decodeJid(m.key.remoteJid || m.message[mtype] && m.message[mtype].groupId || '')
        m.isGroup = m.chat.endsWith('@g.us')
    ...
    if (m.message) {
        ...
        if (m.quoted) {
            ...
            m.getQuotedObj = m.getQuotedMessage = () => {
                ...
                let q = M.fromObject(((XeonBotInc.chats[m.quoted.chat] || {}).messages || {})[m.quoted.id])
             ...
             * @param {Object} options
             */
            m.quoted.reply = (text, chatId, options) => XeonBotInc.reply(chatId ? chatId : m.chat, text, vM, options)

            /**
             * Copy quoted message
             */
            m.quoted.copy = () => exports.smsg(XeonBotInc, M.fromObject(M.toObject(vM)))
            ...
            */
            m.quoted.forward = (jid, forceForward = false) => XeonBotInc.forwardMessage(jid, vM, forceForward)
             ...
             * @param {Object} options
            */
            m.quoted.copyNForward = (jid, forceForward = true, options = {}) => XeonBotInc.copyNForward(jid, vM, forceForward, options)
             ...
             * @param {Object} options
            */
            m.quoted.cMod = (jid, text = '', sender = m.quoted.sender, options = {}) => XeonBotInc.cMod(jid, vM, text, sender, options)
    ...
    m.name = m.pushName || XeonBotInc.getName(m.sender)
    if (m.msg && m.msg.url) m.download = (saveToFile = false) => XeonBotInc.downloadM(m.msg, m.mtype.replace(/message/i, ''), saveToFile)
    /**
     ...
     * @param {Object} options
     */
    m.reply = (text, chatId, options) => XeonBotInc.reply(chatId ? chatId : m.chat, text, m, options)
     ...
     * @param {Boolean} forceForward
     */
    m.forward = (jid = m.chat, forceForward = false) => XeonBotInc.copyNForward(jid, m, forceForward, options)
     ...
     * @param {Object} options
     */
     m.reply = async (text, chatId, options) => XeonBotInc.reply(chatId ? chatId : m.chat, text, m, options)
    ...
    return XeonBotInc.relayMessage(m.chat, msg.message, {
      contextInfo: {
    ...
    m.copyNForward = (jid = m.chat, forceForward = true, options = {}) => XeonBotInc.copyNForward(jid, m, forceForward, options)
     ...
     * @param {Object} options 
     */
    m.cMod = (jid, text = '', sender = m.sender, options = {}) => XeonBotInc.cMod(jid, m, text, sender, options)
    ...
    try {
        if (m.msg && m.mtype == 'protocolMessage') XeonBotInc.ev.emit('message.delete', m.msg.key)
    } catch (e) {
        console.error(e)
    }
    return m
